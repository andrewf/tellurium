...
fn test_hwres() {
    ...
    assert_eq!(hwr.pick_unused_register(), Some(HwLoc::Register("ebx".into())));
}
...
impl Platform for IntelPlatform {
    ...
    fn get_fun_hwreqs(&self, sig: &FunSignature) -> Result<HwReqs, Error> {
        ...
        for range in sig.argtypes.iter()
    ...
    for (stmt, hw) in fun.body.nodes.iter().zip(nodehw) {
        ...
        match stmt.action {
            NodeAction::Call(ref called_sig) => {
                let call_loc = try!(hw.inputs.last().ok_or(CodeGenError::Other("no fn to call".into())));
...
fn register_allocation(plat: &Platform, graph: &FlowGraph)
    -> Result<(u64, Vec<NodeHw>), CodeGenError>
{
    ...
    for node in graph.nodes.iter() {
        ...
        for (hwloc, slot) in reqs.afters.iter().zip(&node.outputs)
                               .filter_map(|(var_id, slot)| reqs.variables[*var_id].concrete()
                                                                .map(|loc| (loc, slot)))
        {
            // range is concrete.
        ...
        for (var_id, slot) in reqs.befores
                            .iter()
                            .zip(&node.inputs)
                            //.filter(|&(var, _)| var.concrete().is_none() )
        {
            let var = &mut reqs.variables[*var_id];
            if var.concrete().is_none() {
                // if 
                let r = try!(needed_befores
                               .pick_unused_register()
                               .ok_or(CodeGenError::Other("used all registers for input".into())));
        ...
        for (var_id, slot) in reqs.afters.iter().zip(&node.outputs)
                                .filter(|&(var_id, _)| reqs.variables[*var_id].concrete().is_some())
        {
                    ...
                    .zip(&node.outputs)
        {
            let var = &mut reqs.variables[*var_id];
            if var.concrete().is_none() {
                let r = try!(needed_afters
                               .pick_unused_register()
                               .ok_or(CodeGenError::Other("used all registers for output".into())));
                needed_befores.set(*slot, r.clone());
                *var = r.into();
            }
        }
        // collect the variables into actual, final lists of hwlocs
        let before_hw : Vec<HwLoc> = try!(
            reqs.befores.iter()
                        .map(|var_id| reqs.variables[*var_id]
                                          .concrete()
                                          .map(|loc| (*loc).clone())
                                          .ok_or(CodeGenError::Other("failed to alloc hw for input".into())))
                        .collect()
        );
        let after_hw : Vec<HwLoc> = try!(
            reqs.afters.iter()
                        .map(|var_id| reqs.variables[*var_id]
                                          .concrete()
                                          .map(|loc| (*loc).clone())
...
fn test_hwloc_ref() {
    ...
    assert_eq!(hwloc_ref(&HwLoc::labelled_var("global")).expect("oops"), "global");
    assert_eq!(hwloc_ref(&HwLoc::stack(16)).expect("oops"), format!("DWORD [esp + 16]"));
}
...
fn test_ra() {
    ...
    let nodes = vec![
        ...
        Node { // call somefun
            ...
            inputs: vec![3, 4, 5],
            outputs: vec![],
            hwreqs: HwReqs::with_befores(vec![HwLoc::Register("eax".into()).into(), HwLoc::Register("ecx".into()).into(), HwRange::new()]),
        },
